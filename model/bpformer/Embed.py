...
class CrossChannelTokenEmbedding(nn.Module):
    ...
    def forward(self, x):
        # x = self.tokenConv(x)
        x1 = x[:, :, 0, :].unsqueeze(2)  #  (batch_size, 1, 1, d_model)
        x2 = x[:, :, 1, :].unsqueeze(2)  #  (batch_size, 1, 1, d_model)

        out1 = self.conv1(x1.squeeze(2))  #  (batch_size, d_model//2, new_sequence_length)
        out2 = self.conv2(x2.squeeze(2))  #
