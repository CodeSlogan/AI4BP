...
class RecurrentCycle(torch.nn.Module):
    ...
    def __init__(self, cycle_len, channel_size):  # 144 2
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.pro = nn.Linear(self.enc_in * self.pred_len, self.pred_len, bias=True)

    def forward(self, x):
        # x: (batch_size, seq_len, enc_in), cycle_index: (batch_size,)
        B, _, _ = x.shape

        cycle_index = torch.zeros(B, dtype=torch.int32)
